...
import { addContestRequest } from "@/lib/contests";
...
export default function ContestSearchPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage("");
    try {
      await addContestRequest(text.trim());
      setMessage("요청이 저장되었습니다. 관리자 페이지에서 확인할 수 있어요.");
    } catch {
      setMessage("Firebase 권한이 막혀 로컬에 저장했습니다. 관리자 페이지에서 로컬 백업으로 확인할 수 있어요.");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="min-h-full overflow-hidden bg-[#F4F2FF] px-6 py-8 md:px-10 lg:px-14">
      <div className="mx-auto w-full max-w-[1180px]">
        ...
        <form
          ...
          className="relative overflow-hidden rounded-[28px] bg-white/95 p-8 shadow-[0_16px_38px_rgba(48,43,99,0.1)] ring-1 ring-[#E6E3F5] backdrop-blur md:p-10"
        >
          ...
          <div className="relative z-10 max-w-[820px]">
            <h1 className="mb-6 text-[44px] font-black leading-tight text-[#6B5DF6] md:text-[56px]">대외활동 및 공모전</h1>
            <p className="text-xl font-black text-slate-800">원하는 대외활동 및 공모전 조건을 입력해 주세요</p>
            <p className="mt-2 text-lg font-bold text-slate-400">관심 분야, 활동 지역, 대상, 마감일 등 원하는 조건을 자유롭게 작성해 보세요.</p>
            ...
            <div className="mt-20">
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder="예: OO전공자가 지원 가능한 부산 지역 공모전과 포트폴리오를 만들 수 있는 대외활동을 추천해줘."
                ...
                />
                <span className="absolute bottom-4 right-5 text-lg font-black text-slate-300">{text.length} / 500</span>
              </div>
            </div>

            {message && <p className="mt-5 rounded-2xl bg-indigo-50 px-4 py-3 text-sm font-bold text-indigo-600">{message}</p>}
            ...
            >
              ...
              {submitting ? "저장 중..." : "AI 추천받기"}
            </button>
          </div>
        </form>

        <button onClick={() => setLocation("/contests")} className="mt-8 flex items-center gap-2 text-2xl font-black text-[#6B5DF6]">

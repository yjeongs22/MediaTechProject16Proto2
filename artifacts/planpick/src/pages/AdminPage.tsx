...
import { getContests, makeBlankContest, saveContests, type ContestInfo } from "@/lib/contests";
...
export default function AdminPage() {
  ...
  async function handleSave() {
    try {
      ...
      setMessage("공모전 정보가 저장되었습니다.");
    } catch {
      localStorage.setItem("planpickContests", JSON.stringify(contests));
      setMessage("Firebase 권한이 막혀 로컬에 저장했어요. Firestore 규칙을 확인해주세요.");
    }
  }
  ...
  return (
    <div className="min-h-full bg-[#F4F2FF] px-6 py-8 md:px-10 lg:px-14">
      <div className="mx-auto grid max-w-[1180px] gap-6 lg:grid-cols-[320px_1fr]">
        <aside className="rounded-3xl bg-white p-5 shadow-[0_12px_30px_rgba(48,43,99,0.08)] ring-1 ring-slate-100">
          ...
          <main className="rounded-3xl bg-white p-6 shadow-[0_12px_30px_rgba(48,43,99,0.08)] ring-1 ring-slate-100">
            ...
            <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
              ...
              <div className="space-y-4">
                <input className="h-12 w-full rounded-2xl border border-slate-200 px-4 text-sm font-bold outline-none focus:border-indigo-500" placeholder="공모전 이름" value={selected.name} onChange={(e) => updateSelected({ ...selected, name: e.target.value })} />
                <input className="h-12 w-full rounded-2xl border border-slate-200 px-4 text-sm font-bold outline-none focus:border-indigo-500" placeholder="신청 날짜" value={selected.date} onChange={(e) => updateSelected({ ...selected, date: e.target.value })} />
                <textarea className="h-28 w-full resize-none rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold outline-none focus:border-indigo-500" placeholder="AI 추천 이유" value={selected.reason} onChange={(e) => updateSelected({ ...selected, reason: e.target.value })} />
                {selected.links.map((link, index) => (
                  <div key={index} className="grid gap-2 md:grid-cols-[150px_1fr]">
                    <input className="h-11 rounded-xl border border-slate-200 px-3 text-sm font-bold outline-none focus:border-indigo-500" value={link.label} onChange={(e) => {
                      const links = [...selected.links];
                      links[index] = { ...link, label: e.target.value };
                      updateSelected({ ...selected, links });
                    }} />
                    <input className="h-11 rounded-xl border border-slate-200 px-3 text-sm font-bold outline-none focus:border-indigo-500" placeholder="https://..." value={link.href} onChange={(e) => {
                      const links = [...selected.links];

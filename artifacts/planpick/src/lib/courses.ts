...
export const defaultCourses: Course[] = [
  ...
  },
  {
    ...
    courseCode: "CS204",
    room: "공학관 405호",
    review: "개념 이해가 중요하며 시험 범위가 넓다는 평가가 있습니다.",
  ...
  },
  {
    ...
    courseCode: "CS305",
    room: "실습실 210호",
    review: "실습 중심이라 결과물이 남고 프로젝트 경험에 도움이 됩니다.",
  ...
  },
  {
    ...
    courseCode: "CS303",
    room: "공학관 302호",
    review: "SQL 실습이 많고 프로젝트와 취업 준비에 유용합니다.",
  ...
  },
  {
    ...
    courseCode: "GE101",
    room: "교양관 103호",
    review: "부담은 크지 않지만 출석과 발표 준비가 중요합니다.",
...
function normalizeCourse(raw: Partial<Course>, index: number): Course {
  const fallback = defaultCourses[index % defaultCourses.length];
  return {
    ...fallback,
    ...raw,
    id: String(raw.id || raw.courseCode || fallback.id || `course-${index}`),
    name: String(raw.name || fallback.name),
    type: String(raw.type || fallback.type),
    day: String(raw.day || fallback.day).slice(0, 1),
    start: String(raw.start || fallback.start),
    end: String(raw.end || fallback.end),
    credit: Number(raw.credit || fallback.credit || 0),
    difficulty: String(raw.difficulty || fallback.difficulty || ""),
    team: String(raw.team || fallback.team || ""),
    courseCode: String(raw.courseCode || fallback.courseCode || ""),
    room: String(raw.room || fallback.room || ""),
    review: String(raw.review || fallback.review || ""),
    syllabus: String(raw.syllabus || fallback.syllabus || ""),
    color: String(raw.color || fallback.color || "#A78BFA"),
  };
}
...
export async function getCoursesAsync(): Promise<Course[]> {
  const dbCourses = await getDataAsync<Partial<Course>[]>("planpickCourses");
